...
public class PageVO {
	...
	//생성자 - 생성될때,(페이지번호, 데이터 개수, 총게시글 수)를 받는다
	public PageVO(int pageNum, int amount, int total) {
		...
		//1.endPage계산
		//현재 조회하는 번호가 1~10이면 ->10
		//현재 조회하는 번호가 11~20이면 -> 20
		//공식: (int)math.ceil(페이지번호 / 화면에 보여질 페이지네이션개수)*화면에보여질 페이지네이션개수
		
		this.endPage = (int)Math.ceil(this.pageNum / (double)10) *10;
		
		//2.startPage계산
		//공식: 끝페이지 번호 - 화면에보여질 페이지네이션 개수 +1;
		this.startPage = this.endPage - 10 + 1;
		
		//3. 실제끝번호 realEnd
		//만약 게시글이 52개라면 ->실제끝번호는 6번
		//공식:(int)math.ceil(전체게시글수/amount)
		...
		//6.next버튼 활성하 여부
		//예시 : 131개 게시물
		//1~10번까지는 endPage = 10 ,realEnd = 14 (true)
		//11~20번까지는 endPage = 14 , realEnd = 14 (false)
